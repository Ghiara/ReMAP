...
toy2d_config = dict(
    ...
    showcase_itr=500,  # training epoch from which to use weights of policy to showcase
    util_params=dict(
    ...
    algo_params=dict(
        ...
        time_steps=25,  # timesteps before current to be considered for determine current task
        ...
        num_transitions_per_episode=400,  # number of overall transitions per task while each epoch's data collection
        max_path_length=400,  # maximum number of transitions per episode in the environment
    ...
    reconstruction_params=dict(
        ...
        temp_folder=f'{os.getcwd()}/temp_cemrl/{time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())}'  # helper folder for storing encoder and decoder weights while training
    ),
    PCGrad_params = dict(
        use_PCGrad=False,
    ...
    dpmm_params=dict(
        save_dir=f'{os.getcwd()}/temp_bnp/{time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())}',
        start_epoch=0,
        gamma0=5.0,
        num_lap=2,
        fit_interval='epoch',
