...
from ReMAP.configs.toy_config import toy_config
from ReMAP.configs.toy2d_config import toy2d_config
...
import ReMAP.scripts.tb_logging as TB
...
def _normalize_output_paths(variant):
    variant['util_params']['base_log_dir'] = _resolve_project_path(
        variant['util_params']['base_log_dir']
    )
    variant['reconstruction_params']['temp_folder'] = _resolve_project_path(
        variant['reconstruction_params']['temp_folder']
    )
    if 'dpmm_params' in variant and 'save_dir' in variant['dpmm_params']:
        variant['dpmm_params']['save_dir'] = _resolve_project_path(
            variant['dpmm_params']['save_dir']
        )
    return variant
...
def main(config=None, name='', ti_option='', gpu=None, use_mp=None, num_workers=None):
    ...
    variant['inference_option'] = 'dpmm' if ti_option == '' else ti_option
    ...
    variant = _normalize_output_paths(variant)
