#!/usr/bin/env python3
"""
Unified tracking evaluation for CEMRL, MELTS, PEARL, and RL2.

This script evaluates the 12 tracking targets grouped into 4 tasks:
  - velocity_backward: [-2.35, -1.75, -1.45]
  - velocity_forward:  [ 1.45,  1.75,  2.35]
  - goal_backward:     [-9.02, -5.10, -3.14]
  - goal_forward:      [ 3.14,  5.10,  9.02]

It reuses the existing per-baseline tracking evaluators under the same folder
and writes unified JSON/CSV summaries under one output directory.
"""
...
from configs.rl2_default import default_config as rl2_default_config
...
def parse_args():
    ...
    parser.add_argument("--cemrl-config", type=str, default="configs/cemrl_cheetah_tigr_config.json")
    parser.add_argument("--melts-config", type=str, default="configs/melts_cheetah_config.json")
    parser.add_argument("--pearl-config", type=str, default="configs/pearl_cheetah_multi_config.json")
    parser.add_argument("--rl2-config", type=str, default="configs/rl2_cheetah_config.json")
