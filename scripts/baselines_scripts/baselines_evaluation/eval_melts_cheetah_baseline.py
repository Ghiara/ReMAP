#!/usr/bin/env python3
"""
Evaluate a trained MELTS agent on cheetah-multi-task environment.

Loads a trained MELTS checkpoint and runs showcase_all evaluation,
then prints per-task-type performance for comparison with our method.

Usage:
    # Find the experiment directory first:
    ls output/melts_baseline/cheetah-multi-task/

    # Then evaluate:
    python eval_melts_cheetah_baseline.py \
        --exp-dir output/melts_baseline/cheetah-multi-task/<TIMESTAMP> \
        --itr 500 \
        --gpu 0
...
def print_results_table(task_returns, method_name='MELTS'):
    print()
    print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluate a trained MELTS agent on cheetah-multi-task'
    )
    parser.add_argument(
        '--exp-dir', type=str, required=True,
        help='Path to the training experiment directory',
    )
    parser.add_argument(
        '--itr', type=int, default=None,
        help='Checkpoint iteration to load (default: latest available)',
    )
    parser.add_argument(
        '--config', type=str,
    ...
    variant['util_params']['exp_name'] = 'melts_eval'
    variant['inference_option'] = 'dpmm'

    print()
    print("=" * 70)
    print("MELTS Evaluation  (showcase_all)")
    ...
    if os.path.exists(results_json):
        ...
        if task_returns:
            print_results_table(task_returns, method_name='MELTS')
