...
def plot_velocity_tracking_curves(vel_results, save_dir):
    """Plot per-timestep velocity curves for each target velocity."""
    ...
    for i, vel in enumerate(velocities):
        if i >= len(axes):
            break
        ax = axes[i]
        data = vel_results[vel]
        trajs = data['achieved_velocities']

        for j, traj_vel in enumerate(trajs):
            alpha = 0.3 if j < len(trajs) - 1 else 1.0
            lw = 1 if j < len(trajs) - 1 else 2
            label = f'Traj {j+1}' + (' (adapted)' if j == len(trajs)-1 else ' (explore)')
            ax.plot(traj_vel, alpha=alpha, linewidth=lw, label=label)
        ...
        ax.set_ylim(min(-1, vel - 3), vel + 3)

    # Remove unused subplots
    for i in range(len(velocities), len(axes)):
        fig.delaxes(axes[i])

    fig.suptitle('PEARL: Velocity Tracking per Timestep', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def plot_goal_tracking_curves(goal_results, save_dir):
    """Plot per-timestep position curves for each target goal."""
    ...
    for i, goal in enumerate(goals):
        if i >= len(axes):
            break
        ax = axes[i]
        data = goal_results[goal]
        trajs = data['achieved_positions']

        for j, traj_pos in enumerate(trajs):
            alpha = 0.3 if j < len(trajs) - 1 else 1.0
            lw = 1 if j < len(trajs) - 1 else 2
            label = f'Traj {j+1}' + (' (adapted)' if j == len(trajs)-1 else ' (explore)')
            ax.plot(traj_pos, alpha=alpha, linewidth=lw, label=label)
    ...
    fig.suptitle('PEARL: Goal Position Tracking per Timestep', fontsize=14, fontweight='bold')
    plt.tight_layout()
