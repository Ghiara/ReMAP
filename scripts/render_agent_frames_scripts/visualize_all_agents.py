"""
How to use:
This scripts helps visualize the showcase frame for hopper, walker, cheetah, ant, 
...
AGENTS = {
    'walker2d': dict(
        cfg_path='output/low_level_policy/walker_multi_new_config_v0_run0/config.json',
    ...
    'hopper': dict(
        cfg_path='output/low_level_policy/hopper_multi_new_config_v3_run0/config.json',
        task_variants=['forward_vel', 'backward_vel', 'goal_front', 'goal_back'],
        task_labels={
            'forward_vel':  '(a) Run forward',
            'backward_vel': '(b) Run back',
    ...
    'ant': dict(
        cfg_path='output/low_level_policy/ant_multi_new_config_v4/config.json',
        task_variants=['goal_front', 'goal_back', 'forward_vel', 'backward_vel'],
        task_labels={
            'goal_front':   '(a) Goal in front',
            'goal_back':    '(b) Goal in back',
            'forward_vel':  '(c) Run forward',
            'backward_vel': '(d) Run back',
        },
        task_colors={
            'goal_front':   [1.0, 1.0, 0.0],
            'goal_back':    [0.0, 1.0, 1.0],
...
def set_task(agent_name, env, cfg, task_name, acfg):
    ...
    if agent_name in ('walker2d', 'hopper'):
        ...
        # Keep the showcase coloring consistent with the other agents.
        geom_rgba = env.model.geom_rgba.copy()
        geom_rgba[2:, :3] = np.array(color)
        env.model.geom_rgba[:] = geom_rgba
