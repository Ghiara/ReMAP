...
#这个脚本是为了渲染walker的多任务视频的，其他环境也可以用这个脚本渲染，只需要指定--agent参数为对应的环境名称即可，
# 比如python render_walker_multitask_videos.py --agent hopper 就可以渲染hopper的多任务视频了。
# 默认情况下会渲染walker的多任务视频，因为这个脚本最初是为了这个目的写的。
