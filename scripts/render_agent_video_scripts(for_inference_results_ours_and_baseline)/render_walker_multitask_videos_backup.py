...
#这个版本是使用toy到对应的位置在附近抖动，但是抖动频率太快了
#这个脚本是为了渲染walker、hopper、ant和toy的训练好的多任务策略视频。
# 对于walker/hopper/ant，视频展示了每个任务的多个目标值，并在屏幕上覆盖了当前目标值、实际值和误差等指标。对于toy，视频展示了一个1D环境中玩
...
AGENT_SPECS = {
    ...
    "hopper": {
        "env_cls": HopperMulti,
        "default_experiment_root": ROOT / "output" / "low_level_policy" / "hopper_multi_new_config_v4",
    ...
    "ant": {
        "env_cls": AntMulti,
        "default_experiment_root": ROOT / "output" / "low_level_policy" / "ant_multi_new_config_v4",
        "default_max_steps": 400,
        "task_name_map": {
            "goal_forward": "goal_front",
            "goal_backward": "goal_back",
...
def render_single_video(
    ...
    for step_idx in range(max_steps):
        ...
        if agent_name == "toy":  # toy patch
            ...
            with torch.no_grad():
                action = policy.get_action(obs_tensor, subgoal, return_dist=False)
...
def main():
    ...
    if args.agent == "toy":  # toy patch
        ...
        # no need to normalize it with fake goal_left/goal_right 
        # and velocity_left/velocity_right tasks that mirror 
        # the front/back and forward/backward tasks, respectively. 
        # Just keep the original config as-is. 
        # The render script will resolve the correct task names 
        # for ant based on what's in the config, 
