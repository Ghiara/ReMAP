...
class AntMulti(AntEnv, utils.EzPickle):
    def __init__(self, render_mode: str = 'rgb_array', *args, **kwargs):
        ...
        self.change_prob = kwargs.get('change_prob', .0)
    ...
    def step(self, action):
        ...
        return ob, reward, done, False, dict(reward_run=reward_run, reward_ctrl=reward_ctrl, true_task=dict(base_task=self.base_task, specification=self.task_specification))
    ...
    def sample_task(self):
        ...
        task = np.zeros(5)
        base_task = np.random.choice(9)
        # velocity tasks
        if base_task == 4:
            vel = np.random.random()*(-self.velocity_x[1])
            self.base_task = self.bt2t['velocity_left']
            self.task_specification = np.array([vel, 0])
            task[0] = vel
        if base_task == 5:
            vel = np.random.random()*self.velocity_x[1]
            self.base_task = self.bt2t['velocity_right']
            self.task_specification = np.array([vel, 0])
            task[0] = vel
        if base_task == 6:
            vel = np.random.random()*self.velocity_x[1]
            self.base_task = self.bt2t['velocity_up']
            self.task_specification = np.array([0, vel])
            task[1] = vel
        if base_task == 7:
            vel = np.random.random()*(-self.velocity_x[1])
            self.base_task = self.bt2t['velocity_down']
            self.task_specification = np.array([0, vel])
            task[1] = vel

        # goal
        if base_task == 0:
            pos = np.random.random()*(-self.pos_x[1])
            self.base_task = self.bt2t['goal_left']
            self.task_specification = np.array([pos, 0])
            task[2] = pos
        if base_task == 1:
            pos = np.random.random()*self.pos_x[1]
            self.base_task = self.bt2t['goal_right']
            self.task_specification = np.array([pos, 0])
            task[2] = pos
        if base_task == 2:
            pos = np.random.random()*self.pos_x[1]
            self.base_task = self.bt2t['goal_up']
            self.task_specification = np.array([0, pos])
            task[3] = pos
        if base_task == 3:
            pos = np.random.random()*(-self.pos_x[1])
            self.base_task = self.bt2t['goal_down']
            self.task_specification = np.array([0, pos])
            task[3] = pos

        # jump
        if base_task == 8:
            goal = np.random.random()*self.velocity_z[1]
            self.base_task = self.bt2t['jump']
            self.task_specification = goal
            task[4] = goal
