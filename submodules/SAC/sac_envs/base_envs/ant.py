...
class AntEnv(MuJocoPyEnv, utils.EzPickle):
    ...
    def __init__(self, **kwargs):
        ...
        xml_path = os.path.join(base_dir, "agent_files", "ant.xml")
    ...
    def _get_obs(self):
        return np.concatenate(
            [
                self.sim.data.qpos.flat[2:],#13 dim
                self.sim.data.qvel.flat,#14 dim
                np.clip(self.sim.data.cfrc_ext, -1, 1).flat,#84 dim
            ]
        )
