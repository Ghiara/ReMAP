...
class RolloutCoordinator:
    ...
    def collect_data(self, tasks, num_samples_per_task=200, num_trajs_per_task=np.inf):
        n_steps_total = 0
        n_trajs = 0
        results = []
        for task in tasks:
            while n_steps_total < num_samples_per_task * len(tasks) and n_trajs < num_trajs_per_task * len(tasks):
                initial_o = self.env.reset()[0]
                _results, _n_steps_total, _n_trajs = self.rollout(task, initial_o)
                results += _results
    ...
    def collect_data2(self, tasks, train_test, deterministic=False, num_samples_per_task=np.inf, num_trajs_per_task=np.inf, animated=False, save_frames=False):
    ...
    def rollout(self, task, initial_o):
        ...
        while path_length < self.max_path_length:
            action = self.agent.get_action(torch.tensor(obs).cuda(), task, return_dist=True)
            new_obs, rew, term, truncated, info = self.env.step
            path['observations'].append(obs)
            path['actions'].append(action)
            path['rewards'].append(rew)
            path['next_obs'].append(new_obs)
            path['terminals'].append(term)
            path['agent_infos'].append(info)
            path['true_tasks'].append[task]
    ...
    def rollout2(self, n_tasks, deterministic, animated, save_frames):
        ...
        while path_length < self.max_path_length:
            ...
            # Step
            actions = self.agent.get_action(ptu.from_numpy(obs), task, return_dist=False)
            # out = self.agent.get_actions(task, ptu.from_numpy(obs), deterministic=deterministic)
