...
META_RL_ROOT = Path(__file__).resolve().parents[2]
REMAP_ROOT = Path(__file__).resolve().parents[4]
for _path in (META_RL_ROOT, REMAP_ROOT):
...
from submodules.SAC.model import PolicyNetwork as TransferFunction
...
DEFAULT_TRANSFER_EXPERIMENT_DIR = Path(
    os.environ.get(
        ...
        str(REMAP_ROOT / 'output' / 'low_level_policy' / 'walker_multi_new_config_v0_run0'),
    )
)
DEFAULT_TRANSFER_EPOCH = int(os.environ.get('META_RL_TRANSFER_EPOCH', '700'))
...
DEFAULT_EVAL_OUTPUT = Path(
    os.environ.get(
        'META_RL_EVAL_OUTPUT',
        str(META_RL_ROOT / 'evaluation' / 'experiments_thesis' / 'transfer_full_walker_5_test'),
    )
)
