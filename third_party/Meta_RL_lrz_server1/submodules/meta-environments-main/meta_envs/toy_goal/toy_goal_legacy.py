"""
This module contains the legacy code for ToyGoalEnv.
ToyGoalEnv is the environment used by Jonas Jürß.

Author(s): 
    Jonas Jürß, Julius Durmann
Contact: 
...
Note:
    This file is a slightly modified version of the environment code from
    Jonas Jürß' SAMRL code.
...
from ..base import MetaEnv

class ToyGoalEnv(MetaEnv):
    def __init__(self, *args, **kwargs):
        ...
        #####################################################
        self._tasks = self.sample_tasks(kwargs['n_train_tasks'] + kwargs['n_eval_tasks'], grid_mode=self.grid_mode)  # is this correct?
        self.train_tasks = self._tasks[:kwargs['n_train_tasks']]
        self.test_tasks = self._tasks[kwargs['n_train_tasks']:]
        self.grid_tasks = self.sample_tasks(kwargs['n_grid_tasks'], grid_mode="cartesian")
        self._tasks += self.grid_tasks
        self.last_idx = None
        self.env_buffer = {}
        self.reset_task(0)
        #######################################################
    ...
    def reset_task(self, idx, keep_buffered=False):
        ...
        self.last_idx = idx
        self._task = self._tasks[int(idx)]
    ...
    def step(self, action):
        ...
        if self.goal_1d:
            ...
            return ob, reward, done, dict(true_task=dict(base_task=0, specification=self.goal['goal'].sum(),
                                                         task_spec=self.goal['goal']),
                                          # np.array([self.goal['goal'][0], self.goal['goal'][1], self.goal['angle'], self.goal['radius']])), TODO enable specification of higher dimension
                                          success=bool(
    ...
    def _get_reward_at_with_goals(self, y, x, y_goal, x_goal):
        ...
        if self.task_goal_offset != 0:
            reward = np.where(np.logical_or(np.minimum(y_goal, y) >= self.task_goal_offset,
                                            np.maximum(y_goal, y) <= -self.task_goal_offset), -dist,
                              -self.task_max_radius)
        else:
            reward = -dist
    ...
    def get_all_task_rewards(self, ys, xs):
        assert ys.shape[0] == len(self._tasks) == xs.shape[0] and ys.ndim == 1 == xs.ndim
        goals = np.array([t['goal'] for t in self._tasks])
    ...
    def sample_tasks(self, num_tasks, grid_mode):
        ...
        tasks = [{'id': id, 'goal': goal, 'angle': angle, 'radius': rv, 'base_task': 0} for id, (goal, angle, rv) in enumerate(zip(goals, a, r))]
        return tasks
    ...
    def sample_task(self):
        min, max = (0, len(self.train_tasks)) if self.meta_mode == "train" else (len(self.train_tasks), len(self.train_tasks) + len(self.test_tasks))
        idx = np.random.choice(np.arange(min, max))
        self.reset_task(idx)
    ...
    def render(self, mode='rgb_array', *args, **kwargs):
        assert mode == 'rgb_array', "Can only render in mode 'rgb_array'."
