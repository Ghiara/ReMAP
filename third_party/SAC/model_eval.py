...
# from experiments_configs.test_new_architecture import config
...
from third_party.Meta_RL.smrl.utility.console_strings import bold, italic
from third_party.Meta_RL.smrl.utility.ops import CustomJsonEncoder
from third_party.Meta_RL.smrl.policies.exploration import RandomMemoryPolicy, RandomPolicy
...
from third_party.Meta_RL.smrl.vae.encoder_decorators.io_modification import InputOutputDecorator
from meta_envs.mujoco.cheetah import HalfCheetahEnvExternalTask
from meta_envs.toy_goal.toy_1d import Toy1D
from third_party.Meta_RL.specific.encoder_transfer import map_cheetah_to_toy1d
from transfer_function.new_PathCollector import TrajectoryGeneratorWithTransferFunction
...
def model_evaluation(
    ...
    simple_env = Toy1D(25,25, min_pos=0, max_pos=10, max_action=1)
    encoder, env, transfer_function = load_model(encoder, config)
    encoder.encoding_mode = 'mean_var'
    encoder.encoding_dim = 2
    # encoder.encoding_mode = 'mean'
    decoder = None
    value_function = None
...
def load_model(encoder, config):
    ...
    encoder = InputOutputDecorator(encoder, map_cheetah_to_toy1d, observation_dim=config['obs_dim_complex'], action_dim=config['act_simple_dim'], latent_dim=1, encoding_dim=1, context_size=5)
    env = HopperGoal()
...
if __name__ == "__main__":
    ...
    for path in paths:
        ...
        experiment_name = 'chane_after_task_250',
        # epoch = 1870,
        epoch = 2370,
