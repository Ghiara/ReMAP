...
import third_party.CARE.bnpy.data
from third_party.CARE.bnpy.ioutil import BNPYArgParser
from third_party.CARE.bnpy.ioutil.BNPYArgParser import FullDataAlgSet, OnlineDataAlgSet
...
def run(dataName=None, allocModelName=None, obsModelName=None, algName=None,
    ...
    third_party.CARE.bnpy.util.NumericUtil.UpdateConfig(**UnkArgs)
...
def _run_task_internal(jobname, taskid, nTask,
    ...
    if isinstance(dataName, str):
        ...
        if looks_like_path:
            ...
            if not os.path.exists(data_path):
                default_dir_plus_data_path = os.path.join(
                    third_party.CARE.bnpy.DATASET_PATH, data_path)
        ...
        InitData = dataName
        DataArgs = dict()
        assert isinstance(Data, third_party.CARE.bnpy.data.DataObj)
        if algName in OnlineDataAlgSet:
    ...
    import third_party.CARE.bnpy.learnalg.ElapsedTimeLogger as ElapsedTimeLogger
...
def loadDataIteratorFromDisk(datapath, ReqArgs, KwArgs, dataorderseed):
    ...
    if 'OnlineDataPrefs' in KwArgs:
        ...
        OnlineDataArgs = dict()
    OnlineDataArgs['dataorderseed'] = dataorderseed
    DataIterator = third_party.CARE.bnpy.data.DataIteratorFromDisk(
        datapath,
...
def getKwArgsForLoadData(ReqArgs, UnkArgs, KwArgs=dict()):
    ...
    '''
    if isinstance(ReqArgs, third_party.CARE.bnpy.data.DataObj):
        datamod = ReqArgs
    else:
        datamod = __import__(ReqArgs['dataName'], fromlist=[])
...
def make_initialized_model(
    ...
    hmodel = third_party.CARE.bnpy.HModel.CreateEntireModel(
...
def createLearnAlg(
    ...
    if algName == 'EM':
        LearnAlgConstr = third_party.CARE.bnpy.learnalg.EMAlg
    elif algName == 'VB':
        LearnAlgConstr = third_party.CARE.bnpy.learnalg.VBAlg
    elif algName == 'pVB':
        LearnAlgConstr = third_party.CARE.bnpy.learnalg.ParallelVBAlg
    elif algName == 'soVB':
        LearnAlgConstr = third_party.CARE.bnpy.learnalg.SOVBAlg
    elif algName == 'memoVB':
        LearnAlgConstr = third_party.CARE.bnpy.learnalg.MemoVBMovesAlg
    elif algName == 'moVB':
        if hasMoves:
            LearnAlgConstr = third_party.CARE.bnpy.learnalg.MOVBBirthMergeAlg
        else:
            LearnAlgConstr = third_party.CARE.bnpy.learnalg.MOVBAlg
    elif algName == 'pmoVB':
        if hasMoves:
            LearnAlgConstr = third_party.CARE.bnpy.learnalg.ParallelMOVBMovesAlg
        else:
            LearnAlgConstr = third_party.CARE.bnpy.learnalg.ParallelMOVBAlg
    elif algName == 'GS':
        LearnAlgConstr = third_party.CARE.bnpy.learnalg.GSAlg
    else:
