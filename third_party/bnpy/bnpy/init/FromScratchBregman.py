...
from third_party.CARE.bnpy.util import split_str_into_fixed_width_lines
from third_party.CARE.bnpy.init.FromTruth import \
...
def initSS_BregmanDiv(
    ...
    if isinstance(Dslice, third_party.CARE.bnpy.data.BagOfWordsData):
...
def makeDataSubsetByThresholdResp(
    ...
    if isinstance(Data, third_party.CARE.bnpy.data.BagOfWordsData):
        return makeDataSubsetByThresholdResp_BagOfWordsData(Data, curModel,
            ...
            **kwargs)
    assert isinstance(Data, third_party.CARE.bnpy.data.XData) or \
        isinstance(Data, third_party.CARE.bnpy.data.GroupXData)
