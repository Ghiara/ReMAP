...
from third_party.CARE.bnpy.data.DataIterator import DataIterator
...
def loadDataFromSavedTask(taskoutpath, dataSplitName='train', **kwargs):
    ...
    try:
        ...
        if dataSplitName == 'train':
            # Load from file
            DI = third_party.CARE.bnpy.data.DataIteratorFromDisk(**dataKwargs)
            if 'batchID' in kwargs and kwargs['batchID'] is not None:
            ...
            try:
                ...
                if os.path.exists(matpath) and matpath.endswith('.mat'):
                    if 'vocab_size' in dataKwargs:
                        Data = third_party.CARE.bnpy.data.BagOfWordsData.LoadFromFile_tokenlist(
                            matpath, vocab_size=dataKwargs['vocab_size'])
                        Data.name = dataName
                    else:
                        Data = third_party.CARE.bnpy.data.GroupXData.LoadFromFile(matpath)
                        Data.name = dataName
...
def loadLPKwargsFromDisk(taskoutpath):
    ...
    dataKwargs : dict with options for loading dataset
    '''
    from third_party.CARE.bnpy.ioutil.BNPYArgParser import algChoices
    chosentxtfile = None
