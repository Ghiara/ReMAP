...
from third_party.CARE.bnpy.viz.PlotUtil import pylab
import third_party.CARE.bnpy.ioutil.BNPYArgParser as BNPYArgParser
import third_party.CARE.bnpy.viz
from third_party.CARE.bnpy.ioutil import ModelReader
from third_party.CARE.bnpy.viz.TaskRanker import rankTasksForSingleJobOnDisk
from third_party.CARE.bnpy.viz.PlotTrace import taskidsHelpMsg
from third_party.CARE.bnpy.viz.PrintTopics import uidsAndCounts2strlist
from third_party.CARE.bnpy.ioutil.DataReader import loadDataFromSavedTask
...
def plotCompsFromHModel(hmodel, **kwargs):
    ...
    if obsName.count('Gauss'):
        if hmodel.obsModel.D > 2:
            third_party.CARE.bnpy.viz.GaussViz.plotCovMatFromHModel(hmodel, **kwargs)
        elif hmodel.obsModel.D == 2:
            third_party.CARE.bnpy.viz.GaussViz.plotGauss2DFromHModel(hmodel, **kwargs)
        elif hmodel.obsModel.D == 1:
            third_party.CARE.bnpy.viz.GaussViz.plotGauss1DFromHModel(hmodel, **kwargs)
        ...
        if 'vocabList' in kwargs and kwargs['vocabList'] is not None:
            ...
            third_party.CARE.bnpy.viz.BernViz.plotCompsFromHModel(hmodel, **kwargs)
    elif obsName.count('Mult'):
        if 'vocabList' in kwargs and kwargs['vocabList'] is not None:
            bnpy.viz.PrintTopics.plotCompsFromHModel(hmodel, **kwargs)
        else:
            third_party.CARE.bnpy.viz.BarsViz.plotBarsFromHModel(hmodel, **kwargs)
...
def plotCompsForTask(task_output_path, lap=None,
    ...
    queryLap = lap
    hmodel, lap = third_party.CARE.bnpy.load_model_at_lap(task_output_path, queryLap)
