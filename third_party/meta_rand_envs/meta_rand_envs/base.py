...
class RandomParamEnv(NonStationaryMetaEnv, MuJocoPyEnv):
    def __init__(self, *args, **kwargs):
        NonStationaryMetaEnv.__init__(self, *args, **kwargs)
        ...
        MuJocoPyEnv.__init__(self, file_name, frame_skip)
...
class RandomMassParamEnv(NonStationaryMetaEnv, MuJocoPyEnv):
    def __init__(self, *args, **kwargs):
        NonStationaryMetaEnv.__init__(self, *args, **kwargs)
        self.file_name = kwargs.get('filename', 'walker2d.xml')
        frame_skip = kwargs.get('frame_skip', 4)
        self.log_scale_limit = kwargs.get('log_scale_limit', 0.0)
        MuJocoPyEnv.__init__(self, self.file_name, frame_skip)
        utils.EzPickle.__init__(self)
        self._init_geom_rgba = self.model.geom_rgba.copy()
        self.save_parameters()
...
class RandomDampParamEnv(NonStationaryMetaEnv, MuJocoPyEnv):
    def __init__(self, *args, **kwargs):
        NonStationaryMetaEnv.__init__(self, *args, **kwargs)
        self.file_name = kwargs.get('filename', 'walker2d.xml')
        frame_skip = kwargs.get('frame_skip', 4)
...
class RandomEnv(MetaEnv, MuJocoPyEnv):
    ...
    metadata = {
        "render_modes": [
            ...
            "rgb_array",
            "depth_array",
        ],     # Depends on the variable timestep in the xml file 
        "render_fps":65
    }

    def __init__(self, log_scale_limit, file_name, *args, observation_space, hfield_mode='gentle', rand_params=RAND_PARAMS, skip_frames=5, **kwargs):
    ...
    def spawn_env(self):
        ...
        try:
            MuJocoPyEnv.__init__(self, self.file_name, self.skip_frames, self.observation_space)
        except:
            ...
            self.metadata['render_fps'] = np.round(1/self.dt)
            MuJocoPyEnv.__init__(self, full_file_name, self.skip_frames, observation_space=self.observation_space)
