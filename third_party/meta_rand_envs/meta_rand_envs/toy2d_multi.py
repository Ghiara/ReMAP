...
class Toy2dMultiTask(RandomEnv, utils.EzPickle):
    def __init__(self, *args, **kwargs):
        self.simple_env_dt = 0.05
        ...
        task_names = ['velocity_up', 'velocity_down',
        ...
        self.model_path = str(Path(__file__).resolve().with_name('toy2d.xml'))
    ...
    def _step(self, action):
        ...
        box_xyz_before = np.array(self.get_body_com("box"))
        try:
        ...
        # goal and direction
            ...
            xposafter = np.array(self.get_body_com("box"))
            ...
            box_xyz_after = np.array(self.get_body_com("box"))
            box_velocity = box_xyz_after - box_xyz_before
            reward_run = np.dot((box_velocity[:2] / self.dt), direct)
