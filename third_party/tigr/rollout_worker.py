...
class RolloutCoordinator:
    def __init__(self,
                 ...
                 gpu_id,
                 scripted_policy,
                 replay_buffer_exploration=None,
                 ):
        self.env = env
        ...
        self.replay_buffer_exploration = replay_buffer_exploration
        ...
        if self.use_multiprocessing:
            ...
            self.workers = [RemoteRolloutWorker.remote(self.env, self.simple_env) for _ in range(self.num_workers)]

        else:
            self.workers = [RolloutWorker(self.env, self.simple_env) for _ in range(self.num_workers)]
    ...
    def rollout(self, n_tasks, deterministic, animated, save_frames):
        ...
        terminals = np.zeros(n_tasks, dtype=bool)
        self.contexts = torch.zeros((n_tasks, self.time_steps, self.obs_space + 1 + self.obs_space))
        ...
        while path_length < self.max_path_length:
            ...
            #     var = 0.5
            #     actions = np.random.normal(loc=actions, scale=var, size=actions.shape)
            ...
            # if path_length == 0:
            #     out = self.agent.get_actions(agent_input, ptu.from_numpy(obs), deterministic=deterministic)
            #     self.a, agent_infos = out[0]
            #     self.ti = out[1]
            #     self.mean = np.random.normal(0, 0.3, self.a.shape[0])
            #     self.var = np.random.exponential(0.3, self.a.shape[0])
                
            # actions = np.random.normal(self.mean, self.var)[:,None]
            # actions = np.random.normal(loc=0, scale=1, size=actions)
            # actions = np.ones_like(actions)
            # actions = np.clip(actions, -1,1)
            # task_indicators = self.ti
            ...
            # if self.simple_env:
            #     obs, actions, next_obs, rewards, new_terminals, env_infos = [list(temp) for temp in zip(*outputs)]
            ...
            terminals[terminals_map] = np.array(new_terminals, dtype=bool)
            ...
            if np.array(new_terminals, dtype=bool).all():
                break

            # Update obs but only take elements that are not terminated!
            obs = next_obs[np.array(new_terminals, dtype=bool) == False]
                ...
                for p_ind, path in enumerate(results):
                    ...
                    for s_ind, info in enumerate(path['env_infos']):
                        images_.append({
                            ...
                            'observations' : path['next_observations'][s_ind],
                        })
    ...
    def update_context(self, o, a, r, next_o):
        ...
        data = torch.cat([o, r, next_o], dim=-1).unsqueeze(dim=1)
...
class RolloutWorker:
    ...
    def step_envs(self, actions, terminals, animated=False, save_frames=False):
        ...
        for i, task_nr in enumerate([self.active_task_list[t_] for t_ in np.where(terminals==False)[0]] if terminals is not None else self.active_task_list):
            ...
            if actions is None:
                output.append(self.env.reset()[0])
                # if self.simple_env:
                #     self.env.sim.data.set_joint_qpos('boxslideZ', 0)
            else:
                # if self.simple_env:
                #     obs, action, next_o, r, d, env_info = self.env.step(actions[i])
                ...
                #     self.env.sim.data.set_joint_qpos('boxslideZ', 0)
                # if self.simple_env:
                #     output.append((obs, action, next_o, r, d, env_info))
...
class RemoteRolloutWorker(RolloutWorker):
    def __init__(self, env, simple_env):
        super().__init__(env, simple_env)
