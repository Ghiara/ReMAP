...
class DecoderMDP(nn.Module):
    ...
    def __init__(self,
        ...
        self.task_decoder_input_size = state_dim + z_dim
...
class ExtendedDecoderMDP(DecoderMDP):
    def __init__(self,
                 action_dim,
                 state_dim,
                 reward_dim,
        ...
        super().__init__(action_dim,
                 ...
                 net_complex,
                 state_reconstruction_clip)
        self.task_decoder = Mlp(
            input_size=self.task_decoder_input_size,
            hidden_sizes=[self.reward_decoder_hidden_size, self.reward_decoder_hidden_size, self.reward_decoder_hidden_size // 2],
            output_size=output_action_dim
        )

    def forward(self, state, action, next_state, z):
        state_estimate, reward_estimate = super().forward(state, action, next_state, z)
        task_estimate = self.task_decoder(torch.cat([state, z], dim=-1))
        return state_estimate, reward_estimate, task_estimate
