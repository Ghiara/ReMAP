...
class AugmentedTrainer(BaseTrainer):
    ...
    def mixture_training_step(self, indices):
        ...
        true_task = np.array([a['base_task'] for a in d_data['true_tasks'][:, -1, 0]], dtype=int)
        ...
        if self.task_given:
            ...
            state_estimate, reward_estimate, task_logits = self.decoder(states, actions, decoder_state_target,
                                                       latent_variables.unsqueeze(1).repeat(1, states.shape[1], 1))
            task_logits = task_logits[:,0]
            # pred = torch.argmax(torch.nn.functional.softmax(task_logits), dim=1)
            # task_loss = pred-true_task
            # indices = np.where(task_loss != 0)[0]
            # task_loss = task_loss[task_loss!=0]
            assert true_task.shape != task_logits.shape[0],'task estimate has not the same dimensions as true_task'
            mixture_task_loss = loss(task_logits, ptu.from_numpy(true_task).long())
        else:
        ...
        if self.task_given:
            mixture_nll += self.loss_weight_task * mixture_task_loss
        ...
        if TB.LOG_INTERVAL > 0 and TB.TI_LOG_STEP % TB.LOG_INTERVAL == 0:
            ...
            if self.task_given:
                total_task_loss = ptu.get_numpy(torch.sum(mixture_task_loss)) / self.batch_size
                TB.TENSORBOARD_LOGGER.add_scalar('training/ti_mixture_task_loss',
                                             total_task_loss.item(), global_step=TB.TI_LOG_STEP)
