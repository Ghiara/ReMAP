...
class TrainingAlgorithm:
    ...
    def train(self):
        ...
        #                                             int(2 ** i_list[i + 0] + 1) + (i == 0)).astype(int)[1:]
            ...
            if epoch in showcase_epochs:
                ...
                for p_nr, path in enumerate(images):
                    ...
                    for im_nr, info in enumerate(path):
                        ...
                        cv2.putText(open_cv_image, f'{path[im_nr]["base_task"] if self.number2name is None else self.number2name[path[im_nr]["base_task"]]}'.upper() + f' | SPEC: {int(info["specification"] * 1000) / 1000 if type(info["specification"]) is np.float64 else [int(k * 1000) / 1000 for k in info["specification"]]}', (0, 15), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (0, 0, 255))
                        cv2.putText(open_cv_image, 'reward: ' + str(info["reward"]) +  'action: ' + str(info["action"]), (0, 35), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (0, 0, 255))
                        # cv2.putText(open_cv_image, 'action: ' + str(info["action"]), (0, 55), cv2.FONT_HERSHEY_TRIPLEX, 0.3, (0, 0, 255))
                        cv2.putText(open_cv_image, 'obs: ' + str(info["observations"]), (0, 55), cv2.FONT_HERSHEY_TRIPLEX, 0.4, (0, 0, 255))
    ...
    def get_gaussian_factors(self, batch_size=1024, num_samples_per_class=100):
        ...
        targets = torch.from_numpy(true_task).long()
    ...
    def generate_augmented_data(self):
        ...
        for step_nr in range(self.augmented_rollout_steps):
            ...
            paths_terminals = np.zeros([obs.shape[0], 1, self.augmented_rollout_length], dtype=bool)
