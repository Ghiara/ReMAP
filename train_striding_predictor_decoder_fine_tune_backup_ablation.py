...
def rollout(env, encoder, decoder, optimizer, simple_agent, step_predictor, transfer_function, memory, 
    ...
    for episode in range(num_trajectory):
        ...
        for path_length in range(max_path_len):
            ...
            # use simple_env to step with simple_action to get the subgoal magnitude
            ...
            if USE_TRUE_TASK:
                ...
                if true_task_idx in [idx_GF, idx_GB]:
                    # position task: magnitude from simple_env.step
                    simple_obs[true_task_idx] = _simple_obs[0].item()

                elif true_task_idx in [idx_FV, idx_BV]:
                    # velocity task: magnitude from simple_env.step
                    v_subgoal = float(np.clip(_simple_obs[1].item(), -3.0, 3.0))
