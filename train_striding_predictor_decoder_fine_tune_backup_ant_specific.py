...
def _balanced_task_for_episode(env, range_dict, episode, rng=None):
    ...
    四类轮流：goal_right, goal_left, velocity_right, velocity_left
    spec 从各自区间随机采样（back 及 backward 取负号）
    """
    if rng is None:
        rng = np.random
    t = env.config['tasks']  # 例如 {'goal_right':0, 'goal_left':1, 'velocity_right':2, 'velocity_left':3}

    order = ['goal_right', 'goal_left', 'velocity_right', 'velocity_left']
    base_name = order[episode % 4]

    if base_name in ['goal_right', 'goal_left']:
        # 位置任务：用你脚本里 range_dict['pos_x'] 的范围
        lo, hi = range_dict['pos_x']                # e.g. [0.1, 1.0]
        spec = float(rng.uniform(lo, hi))
        if base_name == 'goal_left':
            spec = spec
    else:
        # 速度任务：用你脚本里 range_dict['velocity_x'] 的范围
        lo, hi = range_dict['vel_x']           # e.g. [0.1, 1.0]
        spec = float(rng.uniform(lo, hi))
        if base_name == 'velocity_left':
            spec = spec
...
def rollout(env, encoder, decoder, optimizer, simple_agent, step_predictor, transfer_function, memory, 
    ...
    for episode in range(num_trajectory):
        ...
        #     count = 0
        #     while env.base_task not in [env.config.get('tasks',{}).get('goal_right'), 
        ...
        if tasks:
            ...
            # 四类轮流 + 每集随机 spec（亮度）
            #task_cfg(example):{'base_task': 'velocity_right', 'specification': 0.5}
        ...
        for path_length in range(max_path_len):
            ...
            if USE_DECODER:
                ...
                # env 中合法的 task ids
                valid_task_ids = [
                    env.config['tasks']['goal_right'],
                    env.config['tasks']['goal_left'],
                    env.config['tasks']['velocity_right'],
                    env.config['tasks']['velocity_left'],
                ]
            ...
            #simple_obs: [goal_right, goal_left, velocity_right, velocity_left]
            ...
            idx_GF = tasks_cfg['goal_right']
            idx_GB = tasks_cfg['goal_left']
            idx_FV = tasks_cfg['velocity_right']
            idx_BV = tasks_cfg['velocity_left']
            ...
            # if task_prediction in [env.config.get('tasks',{}).get('goal_right'), env.config.get('tasks',{}).get('goal_left')]:
            ...
            #         simple_obs[env.config.get('tasks',{}).get('goal_right')] = _simple_obs[0].item()
            #     else:
            #         simple_obs[env.config.get('tasks',{}).get('goal_left')] = _simple_obs[0].item()
            # else:
            #     if simple_action.item()>0:
            #         simple_obs[env.config.get('tasks',{}).get('velocity_right')] = np.clip(_simple_obs[1].item(), -3,3)
            #     else:
            #         simple_obs[env.config.get('tasks',{}).get('velocity_left')] = np.clip(_simple_obs[1].item(), -3,3)
            ...
            if base_task_pred in [env.config.get('tasks',{}).get('goal_right')]:
                low_level_r = - np.abs(action[env.config['tasks']['goal_right']].detach().cpu().numpy()-env.sim.data.qpos[0])/np.abs(action_normalize)- beta * sim_time_steps
                low_level_r = np.clip(low_level_r, -2, 1)
            elif base_task_pred in [env.config.get('tasks',{}).get('goal_left')]:
                low_level_r = - np.abs(action[env.config['tasks']['goal_left']].detach().cpu().numpy()-env.sim.data.qpos[0] )/np.abs(action_normalize)- beta * sim_time_steps
                low_level_r = np.clip(low_level_r, -2, 1)
            elif base_task_pred in [env.config.get('tasks',{}).get('velocity_right')]:
                low_level_r = - np.abs(action[env.config['tasks']['velocity_right']].detach().cpu().numpy()-env.sim.data.qvel[0])/np.abs(action[env.config['tasks']['velocity_right']].item())- beta * sim_time_steps
                low_level_r = np.clip(low_level_r, -2, 1)
            elif base_task_pred in [env.config.get('tasks',{}).get('velocity_left')]:
                low_level_r = - np.abs(action[env.config['tasks']['velocity_left']].detach().cpu().numpy()-env.sim.data.qvel[0])/np.abs(action[env.config['tasks']['velocity_left']].item())- beta * sim_time_steps
                low_level_r = np.clip(low_level_r, -2, 1)
        ...
        # # === 统计奖励，用于后面的 boxplot ===
        if env.base_task in [env.config.get('tasks',{}).get('goal_right'), env.config.get('tasks',{}).get('goal_left')]:
            # Position task
            rewards_data[current_inference_path_name]['position'].append(episode_reward)
        elif env.base_task in [env.config.get('tasks',{}).get('velocity_right'), env.config.get('tasks',{}).get('velocity_left')]:
            # Velocity task
        ...
        try:
            ...
            # 根据任务类型选轨迹字段
            POS_TASKS = [
                env.config['tasks'].get('goal_right'),
                env.config['tasks'].get('goal_left')
            ]
            VEL_TASKS = [
                env.config['tasks'].get('velocity_right'),
                env.config['tasks'].get('velocity_left')
            ]
    ...
    if TRAIN_STRIDE and len(decoder_loss_history) > 0:
        ...
        task_loss_df = pd.DataFrame(dict(
            goal_right = task_loss_hist[0],
            goal_left = task_loss_hist[1],
            velocity_right = task_loss_hist[2],
            velocity_left = task_loss_hist[3],
        ))
...
if __name__ == "__main__":
    ...
    '''
    # taskid: order = ['goal_right:0', 'goal_left:1', 'velocity_right:2', 'velocity_left:3']
    tasks = [
        {'base_task':'goal_left', 'specification':0.9},#1
        {'base_task':'goal_left', 'specification':0.5},
        {'base_task':'goal_left', 'specification':0.3},
        {'base_task':'goal_right', 'specification':0.3},#0
        {'base_task':'goal_right', 'specification':0.5},
        {'base_task':'goal_right', 'specification':0.9},
        {'base_task':'velocity_left', 'specification':0.9},#3
        {'base_task':'velocity_left', 'specification':0.5},
        {'base_task':'velocity_left', 'specification':0.3},
        # {'base_task':'velocity_left', 'specification':1.0},
        # {'base_task':'velocity_right', 'specification':1.0},
        {'base_task':'velocity_right', 'specification':0.3},#2
        {'base_task':'velocity_right', 'specification':0.5},
        {'base_task':'velocity_right', 'specification':0.9},
                ]
    rollout(env, encoder, decoder, optimizer, simple_agent, step_predictor,
                                    transfer_function, memory, variant, obs_dim, action_dim, 
