...
def _write_tensorboard_step_multi(
    save_root,
    step_idx,
    mu_list,
    input_list,
    ...
    step_root = os.path.join(save_root, f"{step_idx:05d}")

    def write_one(name, data):
        if len(data) == 0:
            return

        sub_dir = os.path.join(step_root, name)
        os.makedirs(sub_dir, exist_ok=True)

        np.savetxt(
            os.path.join(sub_dir, "tensors.tsv"),
            np.vstack(data),
            delimiter="\t"
        )

        with open(os.path.join(sub_dir, "metadata.tsv"), "w", newline="") as f:
            writer = csv.writer(f, delimiter="\t")
            for t, s, p in zip(true_ids, specs, pred_ids):
                writer.writerow([f"{int(t)} [{float(s):.3f}] -> {int(p)}"])

    # === 只写两个 ===
    write_one("mu", mu_list)
    write_one("input", input_list)
...
def _goal_biased_train_stride_task_for_episode(env, range_dict, episode, rng=None):
    """
    Train-stride 专用采样器：
    1. 显式过采样 goal_front / goal_back，缓解 showcase 中
       "goal 被错分成同方向 velocity" 的问题。
    2. 对 goal 任务优先采更难的 specification（更靠近区间上界），
       让 decoder 在微调时更多见到“大幅 goal tracking” 的样本。

    返回格式与 _balanced_task_for_episode 保持一致：
    {'base_task': <str>, 'specification': <float>}
    """
    if rng is None:
        rng = np.random
    ...
    # PATCH(goal sampling):
    # 这里把 train_stride 的 episode 分布改成 6-slot 周期：
    #   goal_front, goal_back, goal_front, goal_back, forward_vel, backward_vel
    # 相比原来的四类严格均衡，这会让 goal 类样本占到 4/6，
    # 专门强化 decoder 对 goal-vs-velocity 的区分。
    order = [
        'goal_front',
        'goal_back',
        'goal_front',
    ...
    base_name = order[episode % len(order)]

    if base_name in ['goal_front', 'goal_back']:
        lo, hi = range_dict['pos_x']

        # PATCH(hard goal sampling):
        # 大多数 goal 样本从区间上半段采样，少量样本保留全范围随机，
        # 这样既能增加难 goal 的曝光，又不会完全丢掉容易样本。
        hard_lo = lo + 0.65 * (hi - lo)
        if rng.rand() < 0.7:
            spec = float(rng.uniform(hard_lo, hi))
        else:
            spec = float(rng.uniform(lo, hi))
    else:
        lo, hi = range_dict['vel_x']
        spec = float(rng.uniform(lo, hi))
...
def rollout(env, encoder, decoder, optimizer, simple_agent, step_predictor, transfer_function, memory, 
    ...
    for episode in range(num_trajectory):
        ...
        if tasks:
            # 你自己传了 tasks 列表就按它来
            # 支持两种格式：
            # 1) {'base_task': ..., 'specification': ...}  -> 仍走 env.sample_task 映射
            # 2) {'base_task': ..., 'target_value': ...}   -> 直接使用真实 target 值
            task_cfg = tasks[episode % len(tasks)]
        else:
            # PATCH(train_stride sampling):
            # 只在 train_stride 阶段强化 goal 采样，让 decoder 微调时
            # 更频繁看到 goal_front / goal_back，重点修复 showcase 中
            # "goal tracking -> same-direction velocity tracking" 的误判。
            if TRAIN_STRIDE:
                task_cfg = _goal_biased_train_stride_task_for_episode(env, range_dict, episode)
            else:
                # decoder_eval / oracle_eval 仍保留原来的均衡采样逻辑
                # 方便和旧结果对齐，不把评估分布也一起改掉。
        ...
        if 'target_value' in task_cfg:
            task = np.zeros(max(env.config['tasks'].values()) + 1, dtype=float)
            task[true_task_idx] = float(task_cfg['target_value'])
            env.task = task
            true_goal_value = float(task[true_task_idx])
            true_spec = true_goal_value
            spec_for_logging = true_goal_value
        else:
        ...
        for path_length in range(max_path_len):
            ...
            # Normalize all decoder/policy inputs to 1D vectors so action_dim=1
            # does not collapse into a scalar via squeeze().
            simple_obs_tensor = ptu.from_numpy(simple_obs_before).reshape(-1)
            mu_vec = mu.reshape(-1)

            # Save latent vars
            #simple_obs_before: [pos_x, vel_x]  mu:[mu_1, mu_2, ..., mu_d]
            #policy_input: [pos_x, vel_x, mu_1, mu_2, ..., mu_d]
            policy_input = torch.cat([simple_obs_tensor, mu_vec], dim=-1)
            ...
            simple_action_vec = simple_action.reshape(-1)


            # merge the information for the t-SNE visualization of the decoder input space (simple_obs_before + simple_action + mu)
            tsne_decoder_input = torch.cat([
                simple_obs_tensor,
                simple_action_vec,
                mu_vec
            ], dim=-1)

            export_decoder_input.append(tsne_decoder_input.detach().cpu().numpy())
            ...
            if USE_DECODER:
                _, _, logits = decoder(simple_obs_tensor, simple_action_vec, 0, mu_vec)
            ...
            if USE_TRUE_TASK:
                ...
                raw = float(simple_action_vec[0].detach().cpu())      # [-1, 1]
            ...
            "simple_action": float(simple_action_vec[0].detach().cpu()),
